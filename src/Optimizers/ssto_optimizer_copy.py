...
from Helpers.ssto_guess import sstoGuess
...
class sstoOptimizer: #single phase with constant thrust
    ...
    def set_pbm_driver(self):
        ...
        p.driver.options['dynamic_simul_derivs'] = self.settings['dynamic_simul_derivs']
    ...
    def set_constraints(self, phase, bcs):
        ...
        """
        returns a Phase class instance with all the constraints set
        """
        
        #impose final BCs on the state variables
        ...
        #phase.add_path_constraint('r', lower=bcs[0,0], upper=bcs[0,1], scaler=self.s[1])
...
class sstoOptimizer2p(sstoOptimizer): #two phases (vertical rise and free-attitude) with constant thrust
    ...
    def set_vert_constraints(self, phase, bcs):
        ...
        if self.settings['fixed']=='time':
            phase.set_time_options(fix_duration=True)
        elif self.settings['fixed']=='alt':
            phase.add_boundary_constraint('r', loc='final', equals=bcs[0,1])
    ...
    def run_optimizer(self, F, w, bcs, t0): #t0=(lower_bound, vert_tof, tot_tof, upper_bound)
