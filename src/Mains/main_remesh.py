# -*- coding: utf-8 -*-
"""
The script defines the required constants and settings to numerically solve the optimal control problem for the optimal
two-phases descent trajectory from a circular Low Lunar Orbit (LLO) to the Moon surface

referenced papers:
    
        Remesh et al., "Fuel Optimum Lunar Soft Landing Trajectory Design Using Different Solution Schemes",
    International Review of Aerospace Engineering (IREASE), 2016
...
debug = False #check the partial derivatives defined in the ODEs
...
if ch==1: #single thrust (Remesh)
    ...
    t0 = (1200., 1300.) #time of flight guesses (s)
...
if ch==2: #multiple thrust levels (Remesh)
    ...
    F = np.array([880.0, 1320.0, 1760.0, 2200.0, 2640.0]) #thrust array (N)
    twr = F/W0 #initial thrust/weight ratio array
    hp = 15e3 #intermediate orbit periapsis altitude (m)
    hs = 3.003e3 #switch altitude between horizontal and vertical braking phase (m)    
    t0 = np.array([[1700.0, 1800.0],[1200.0, 1300.0],[650.0,700.0],[500.0,550.0],[400.0,450.0]]) #tof guesses
...
if ch==3: #single thrust (Ramanan)
    ...
    twr = F/W0 #initial thrust/weight ratio
    hp = 15e3 #intermediate orbit periapsis altitude (m)
    hs = 4e3 #switch altitude between horizontal and vertical braking phase (m)
...
if ch==6: #multiple thrust levels saved trajectories
    ...
    a.set_results(d)
    a.display_summary()
    a.plot_all(a.hist, a.idx_opt, a.F, a.rp, a.nbh)