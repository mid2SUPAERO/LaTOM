...
settings = {'solver':solver.upper(), 'transcription':transcription, 'num_seg':num_seg, 'visible_moon':visible_moon,
            ...
            'attitude_rate_bounds':attitude_rate_bounds, 'final_bcs':final_bcs, 'exp_sim':exp_sim}

#parameters
Isp = 450. #Isp (s)
twr = 2.1 #thrust/initial weight ratio
kmin = 1.  #minimum thrust throttle / If kmin=1 constant thrust

#initial guess
t0 = np.array([0.1, 0.5, 2.])*1e3 #time of flight initial guess and bounds (s)
u0 = np.array([[0.2, 0.0, -0.8],[0.5, 0., 0.2]]) #thrust direction initial guess as [[ux0, uy0, uz0], [uxf, uyf, uzf]]
R0 = np.array([0.0, 0.0, -R]) #initial position R0 defined as (x0, y0, z0)

#target orbit coe defined as (a, e, i, raan, w, ta)
rp = R*1.05
e = 0.0
a = rp/(1-e)
coe = np.array([a, e, 90.0, 0.0, 270.0, 10.0])
...
if ch==1: #compute trajectory
    ...
    d = a3d.get_results()
    savemat('../data_3D/3Dtrajectory_case1',d)

elif ch==2: #saved data
    
    d = loadmat("../data_3D/3Dtrajectory_case6", squeeze_me=True, mat_dtype=True)
    a3d = sstoAnalyzer_3D(const, settings)
    a3d.set_params(Isp, twr, kmin, t0, u0)
    a3d.set_results(d)
