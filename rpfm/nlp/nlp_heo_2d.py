...
class TwoDim3PhasesLLO2HEONLP(MultiPhaseNLP):
    ...
    def __init__(self, body, sc, alt, rp, t, alpha_bounds, t_bounds, method, nb_seg, order, solver, ph_name,
        ...
        self.tof_adim = np.reshape(np.array([self.guess.pow1.tf, self.guess.ht.tof,
                                             (self.guess.pow2.tf - self.guess.pow2.t0)]), (3, 1))/self.body.tc
        ...
        if t_bounds is not None:
            ...
            self.phase[0].set_time_options(fix_initial=True, duration_ref0=0.0, duration_ref=self.tof_adim[0])
            self.phase[1].set_time_options(initial_ref0=0.0, initial_ref=self.tof_adim[0],
                                           duration_ref0=0.0, duration_ref=self.tof_adim[1])
            self.phase[2].set_time_options(initial_ref0=0.0, initial_ref=(self.tof_adim[0] + self.tof_adim[1]),
                                           duration_ref0=0.0, duration_ref=self.tof_adim[2])
        ...
        # constraint on injection
        h = self.guess.ht.arrOrb.ra*self.guess.ht.arrOrb.va*self.body.tc/self.body.R**2
        ...
        for i in range(3):
            sn, cn, ts, tc, idx = self.set_time_phase(ti[i, 0], self.tof_adim[i, 0], self.phase[i], self.phase_name[i])
            self.set_initial_guess_phase(tc, idx, self.phase_name[i])
    ...
    def set_initial_guess_phase(self, t_control, idx_state_control, phase_name):

        self.guess.compute_trajectory(t_eval=t_control*self.body.tc)
        ...
        self.p[phase_name + '.controls:alpha'] = np.reshape(self.guess.controls[:, 1], (len(t_control), 1))
